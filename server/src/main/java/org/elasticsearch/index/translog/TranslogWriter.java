...
public class TranslogWriter extends BaseTranslogReader implements Closeable {
    ...
    @Override
    public TranslogIndexedSnapshot newIndexedSnapshot(final LongLongMap index) {
        // we have to make a copy, otherwise this map could be concurrently modified while we are reading from it
        return snapshot(() -> super.newIndexedSnapshot(new LongLongHashMap(index)));
    }
    ...
    private <T> T snapshot(final Supplier<T> supplier) {
        ...
        synchronized (syncLock) {
            synchronized (this) {
                ...
                /*
                 * This must run under lock; see newIndexedSnapshot where we make a copy of the translog index and this must be done when we
                 * are certain that there are no additional modifications being made to the map as it is not thread safe.
                 */
                return supplier.get();
            }
        }
