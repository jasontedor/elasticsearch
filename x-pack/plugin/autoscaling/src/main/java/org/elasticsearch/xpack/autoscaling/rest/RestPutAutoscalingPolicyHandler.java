...
public class RestPutAutoscalingPolicyHandler extends BaseRestHandler {
    ...
    public List<Route> routes() {
        return List.of(new Route(PUT, "/_autoscaling/policy/{tier}"));
    }
    ...
    protected RestChannelConsumer prepareRequest(final RestRequest restRequest, final NodeClient client) throws IOException {
        final String tier = restRequest.param("tier");
        final PutAutoscalingPolicyAction.Request request;
        try (XContentParser parser = restRequest.contentParser()) {
            request = PutAutoscalingPolicyAction.Request.parse(tier, parser);
        }
